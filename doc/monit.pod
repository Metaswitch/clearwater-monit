...
pattern matching to find the process to monitor. The first-match and
top-most-parent is used, so this form of check is most useful if the
process name is unique. Pid-file should be used where possible as it
defines expected PID exactly. You can test if a process match a pattern
...
can use either --no-color option or the following statement
in the monit configuration file to disable the colors completely:

 SET TERMINAL < COLOR | NOCOLOR >



=head1 TERMINAL TABLES

Monit uses UTF-8 box drawing characters for output formating,
if the terminal is set to UTF-8.

If you want to process the monit CLI output in a script, you
can use either --no-table option or the following statement
in the monit configuration file to disable the tables completely:

 SET TERMINAL < TABLE | NOTABLE >
...
execute a start/stop/restart program or an exec action, it will
...
the action. The following and I<only> the following environment
...
I<check process> service context:
...
I<check program> service context:
