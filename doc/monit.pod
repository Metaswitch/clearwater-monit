...
The start delay option allows to postpone the first check by given
number of seconds (can be useful for example when the system boots).
By default Monit starts service checks immediately after start.
...
Monit applies default limits on various tests, to limit the ammount of
data during testing.
...
Where the I<unit> is "B" (byte), "kB" (kilobyte) or "MB" (megabyte).
...
Options legend:
 ...
 | fileContentBuffer | limit for file content test         | 512 B   |
 | httpContentBuffer | limit for HTTP content test         | 1 MB    |
