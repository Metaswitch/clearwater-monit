...
service and in case of problems raise alerts and restart the service.
...
In I<start> mode (B<DEFAULT>), Monit will always start the service automatically
on reboot, even if it was stopped before it.

In I<nostart> mode, the service is I<never> started automatically after reboot.
...
In I<laststate> mode, a service monitoring state is persistent across reboot, i.e. if
...
The name of the service (from monitrc) for which is the program
executed.
