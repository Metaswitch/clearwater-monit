...
You can set SSL/TLS options using an SSL statement (you can also use
TLS instead of SSL for convenience).
...
I<VERSION> allows to set specific SSL/TLS version to use. By default Monit uses
AUTO, which automatically selects the protocol to use. In AUTO mode, only TLS
protocols family is used - if you need to use SSLv2 or SSLv3, you have to explicitly
set the version to it (as it is considered obsolete from the security point of view).

I<VERIFY> allows to enable SSL server certificate verification. If the certificate
is not trusted, not valid yet or expired, Monit will report error. By default
the certificate verification is disabled, we B<strongly recommend> to enable it.

I<SELFSIGNED> by default self-signed certificates are rejected. You can allow self-signed
certificates, but we recommend to do it only selectively in local context where you
want to trust it.

I<CLIENTPEMFILE> sets a path to SSL client certificates database in PEM file format.
When an SSL server requires client certificate authentication, Monit will try to
find a client certificate which corresponds to server's CA from this database and if
found, it will use it for authentication.

I<CACERTIFICATEPATH> sets a path to CA certificates directory. Monit uses OpenSSL's
default directory if this options is not used (you can use "openssl version -d" command
to get the default directory path).

The SSL/TLS options statement can be used either globally (sets default for every SSL/TLS
connection) or locally in the connection test, mailserver or mmonit statement context, which
overrides or extend the global settings.

To set the global SSL/TLS options, use this statement in the global context with a SET keyword:

    set ssl options {...}

Example of global and local SSL/TLS options:

    set ssl options {
            verify: enable
    }

    # verify certificates (via global setting), no self-signed certificates are trusted by default:
    check host myhost1 with address www.myhost1.com
            if failed
                    port 443
                    protocol https
            then alert

    # verify certificates (via global setting), self-signed certificates are trusted locally:
    check host myhost2 with address www.myhost2.com
            if failed
                    port 443
                    protocol https
    ...
    # do not verify certificates (override global setting)
    check host myhost3 with address www.myhost3.com
            if failed
                    port 443
                    protocol https
...
You can enable SSL/TLS with custom L<options|"SSL OPTIONS">. It is
also possible to check a SSL certificate checksum.
...
services, where pre-requisite services are tested first.
...
I<type: TYPE [TCP | UDP]>. Optionally specify the socket type
Monit should use when trying to connect to the port. The
different socket types are: TCP or UDP, where TCP is a
regular stream based socket, UDP is a datagram socket. The
default socket type is TCP.

I<ssl: [SSL | TLS] [with options {...}]>. Enable SSL/TLS. You can
set custom SSL/TLS L<options|"SSL OPTIONS">, such as SSL/TLS version
to use, whether to verify certificates, trust self-signed certificates
or set SSL client certificates database path for client certificates
authentication.

I<certificate checksum: CERTIFICATE CHECKSUM [MD5|SHA1] hash>. Verify the
SSL server certificate by checking its checksum. You can use either MD5
or SHA1 checksum (if you don't specify the type, Monit will determine the
digest based on the hash length). You can use an I<openssl> CLI to get the
checksum value for your certificate, which you can use in Monit's control file:
...
I<certificate expire days: CERTIFICATE EXPIRE in number DAYS>. Send alert
if the certificate is going to expire within the given number of days. This
test allows you to be notified that certificate renewal is necessary.
...
Example:
  ...
  if failed
      port 443
      protocol https
      and certificate expires in 30 days
  then alert
