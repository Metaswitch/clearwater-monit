...
considered obsolete. If you have to use SSLv2 or SSLv3, you must to
...
speaks with the server you think it speaks with. That is, not enabling
certificate verification opens up for a I<man-in-the-middle> attack,
which in Monit's case might not be as critical as when you connect to
your online bank with your Browser. Still, when you use I<check host>
and test a I<https> connection it is good to know that Monit will
verify that it connects to the correct server.
...
directory if this options is not used (you can use "openssl version -d"
command to get the default directory path). Many distributions comes
with SSL and CA certificates already setup and using this option is
normally not necessary.

The SSL options statement can be used both globally (default for all
SSL/TLS connection made through Monit) or local in a check, in
mailserver settings or with the mmonit statement, and will then
...
I<certificate expire days: CERTIFICATE EXPIRE in number DAYS>. Send
alert if the certificate will expire within the given number of days.
...
Example:
  ...
  if failed
      port 443
      protocol https
      and certificate expire in 30 days
  then alert
...
variables which can be utilised by the executable. The following
