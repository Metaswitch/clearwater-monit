...
Example:
 ...
 set mail-format {
      from: Monit <monit@foo.bar>
  reply-to: Support <support@domain.com>
...
The I<from:> option is the sender's email address for Monit alerts.
A sender name is optional and plain address with or without angle
...
By default at maximum 1MB is inspected. You can increase the limit
using the L<set limits|"LIMITS"> statement.
...
I<PASSWORD> SMTP password (special characters can be used, but for non-alphanumerics
the password has to be quoted).

The credentials are B<optional> and when used, it is strongly recommended to use the SMTPS
protocol to protect the credentials. If no credentials are set, Monit will perform just basic
test.
...
Advanced test with STARTTLS and authentication:
